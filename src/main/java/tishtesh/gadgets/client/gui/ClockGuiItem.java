...
public class ClockGuiItem extends GadgetGuiItem{
    ...
    protected void render(Minecraft mc, GadgetOverlayGui gui, int x, int y) {
        ...
        if (Config.CLIENT.gadgetGuiIcons.get()) {
            ...
            switch (mc.player.dimension.getId()) {
                case -1:
            ...
            gui.drawTexture(icon, x + 1, y + (getHeight() / 2) - 9, 18, 18);
            width += 18;
        }

        // Get time details
        float dayTime = mc.world.getDayTime() / 24000F;
        ...
        // Get day
        long day = mc.world.getDayTime() / 24000L;
        String dayString = String.format("Day %d", day);

        // Render time string
        mc.fontRenderer.drawStringWithShadow(timeString, x + 1 + width / 2F - mc.fontRenderer.getStringWidth(timeString) / 2F,
        ...
        mc.fontRenderer.drawStringWithShadow(dayString, (x + 2 + width / 2F - mc.fontRenderer.getStringWidth(dayString) / 2F) / GadgetOverlayGui.SmallTextSF,
